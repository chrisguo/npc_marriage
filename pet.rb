...
以《大明浮生记》游戏为例。
《大明浮生记》的伙伴等级与经验对应表见当前目录下的da_ming_level_exp.csv文件。

伙伴升级到40级，总共需要的经验是：
Exp(L2)= sum(daming_exp[1:40,]$need2next)= 548959

伙伴升级到50级，总共需要的经验是：
Exp(L1)= sum(daming_exp[1:50,]$need2next)= 2638959

则一次结婚，耗费玩家的总经验下限为：
Exp(L1)+Exp(L2)=3187918
它相当于单独把某伙伴的等级升级到51级要的总经验。老郭批注：好像低了点？反论点：这样结婚门槛低，才能鼓励结婚啊。

则十次结婚，耗费玩家的总经验下限为：
10*(Exp(L1)+Exp(L2))=31879180
它相当于单独把某伙伴的等级升级到68级要的总经验。老郭批注：好像低了点？


《大明》结婚设计要点：

一. 使用本代码体现的核心机制，但去掉初始数值的变化，只有成长值的变化。
二. 对于玩家，结婚系统要简单易懂。因此，界面要极为简化，规则要少而容易记忆：
   1. 伙伴50级、40级即可结婚。结婚时无性别差异。
   2. 主将和副将必须是同一职业，结婚诞生的新宝宝还是这个职业。
